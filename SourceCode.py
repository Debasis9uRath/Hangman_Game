...
word_list= ['COMPUTER','WORLD','KEYBOARD','EXIT','CURRENT','HUMANITY','STRONG','CLIMB','MATCH','RESULT','BROTHER','DESIGN','OVER',
            'STUDENT','UNIVERSITY','FRIENDSHIP','WINDOW','FOCUS','PRESENT','HONEST','HONOUR','AVAILABLE','SPEED','REALLY','ALIVE','CLIENT',
            'POPULAR','GAME','WONDERFUL']
		...
		if the_word_withSpaces.count(letter)>0:
			for c in range(len(txt)):
				...
				if lblWord.get()==the_word_withSpaces:
					messagebox.showinfo("Congratulation","You guessed it!")
		else:
			numberOfGuesses += 1
			imgLabel.config(image=photos[numberOfGuesses])
			if numberOfGuesses==11:
					messagebox.showwarning("You lose",the_word_withSpaces)
